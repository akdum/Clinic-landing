...
export class SendGridApi {
    static GetQueryString(userId: string, api_key: string, to: string, subject: string, text: string, from: string): string {
        return `api_user=${encodeURI(userId)}&api_key=${encodeURI(api_key)}&to=${encodeURI(to)}&subject=${encodeURI(subject)}&text=${encodeURI(text)}&from=${encodeURI(from)}`;
    }
    ...
    static async SendMail(data: IMailBody) {
        ...
        var result = await WebRequest.get(this.GetUrl(text));
        return result;
    }
} 